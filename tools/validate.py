...
BANNED = re.compile(
    r"\b(load[- ]bearing|honest(ly|y)?|authentic(ity|ally)?|inauthentic|purist|tourist(s|y)?|touristy"
    r"|real donuts?|true donuts?|proper donuts?|real doughnuts?|the real thing|sacrile\w+)\b", re.I)
...
def validate_all(strict=False, quiet=False) -> int:
    ...
    for r in recs:
        ...
        pf = r.get("profile") or {}
        if pf.get("source") and pf["source"] not in sources:
            errors.append(f"{tag}: profile.source {pf['source']} not in sources.json")
