...
if [ -n "$DOMAIN" ]; then SITE_URL="https://$DOMAIN"; else SITE_URL="https://nanobotco.github.io/pink-box"; fi

STYLE="$HOME/.claude/bin/stylecheck.py"
if [ -f "$STYLE" ]; then
  python3 "$STYLE" tools data schema README.md README.txt AUTHORING.txt BRIEF.txt || {
